...
def get_SR_result(image_file, scale, model, denoise=False):
    ...
    image_ori = pil_image.open(image_file).convert('RGB')
    imgType = imghdr.what(image_file)
    imgType='.'+imgType
    image=image_ori.resize((image_ori.width // scale, image_ori.height // scale), resample=pil_image.Resampling.BICUBIC)
    image_file=image_file.replace(imgType, '_resize_x{}.png'.format(scale))
    ...
    if model=='SRCNN':
        ...
        if denoise==True:
            ...
            SR_result.save(image_file.replace(imgType, '_SRCNN_x{}_denoised.png'.format(scale)))
        else:
            SR_result.save(image_file.replace(imgType, '_SRCNN_x{}.png'.format(scale)))
    elif model=='SROP':
        SR_result=get_srop(image_file, scale)
        if denoise==True:
            SR_result=img_Denoise(SR_result)
            SR_result.save(image_file.replace(imgType, '_SROP_x{}_denoised.png'.format(scale)))
        else:
            SR_result.save(image_file.replace(imgType, '_SROP_x{}.png'.format(scale)))
    elif model=='Bicubic':
        SR_result = image.resize((image.width * scale, image.height * scale), resample=pil_image.Resampling.BICUBIC)
        SR_result.save(image_file.name.replace(imgType, '_bicubic_x{}.png'.format(scale)))
